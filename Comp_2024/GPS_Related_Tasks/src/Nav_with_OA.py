...
latitude = 27.37390764
longitude = -82.45315448
...
class WaypointNavigator(Node):
    ...
    def scan_callback(self, msg):
        ...
        front_distances = filtered_distances[:degree_range_indices]
        back_distances = filtered_distances[-degree_range_indices:]
    ...
    def navigate_to_waypoint(self, distance, angle):
        ...
        if self.degrees_true is not None:
            ...
            # Reset the steady counter if not reached the waypoint yet
                ...
                if OA is False:
                    print("Obstacle Avoidance is defaulting!")
                    obstacle_avodiance = ObstacleAvoidance()
                    rclpy.spin_once(obstacle_avodiance)
                    obstacle_avodiance.destroy_node()  
                else:
