  ...
  zed_topic_benchmark_interfaces__msg__BenchmarkStatsStamped__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    zed_topic_benchmark_interfaces__msg__BenchmarkStatsStamped * data =
      (zed_topic_benchmark_interfaces__msg__BenchmarkStatsStamped *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!zed_topic_benchmark_interfaces__msg__BenchmarkStatsStamped__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          zed_topic_benchmark_interfaces__msg__BenchmarkStatsStamped__fini(&data[i]);
        }
        free(data);
        return false;
      }
