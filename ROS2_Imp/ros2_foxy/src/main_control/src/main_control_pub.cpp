...
//This is where the publisher node is created. In its working state now it creates a string based node called "detector_publisher_node"
    MCPublisherNode() : Node("main_control_publisher_node") {
        //Creates the publisher which is called detector with a buffer of 10
        publisher_ = create_publisher<std_msgs::msg::String>("main_control", 10);
        //Creates a timer which allows publisher to update every 1000ms
        timer_ = create_wall_timer(std::chrono::milliseconds(1000), std::bind(&MCPublisherNode::publishMCData, this));
    }
...
//Here is where the python script gets executed 
    std::string executePythonScript() {
        ...
        std::string command = "python3 ~/RoboBoat_Cyber_Minority/ROS2_Imp/ros2_foxy/src/main_control/MainControl.py";
...
//logs that the code execution was a success and what is being sent to to the publishMCData function
        pclose(pipe);
...
int main(int argc, char *argv[]) {
    ...
    rclcpp::spin(std::make_shared<MCPublisherNode>());
    rclcpp::shutdown();
    return 0;
}
