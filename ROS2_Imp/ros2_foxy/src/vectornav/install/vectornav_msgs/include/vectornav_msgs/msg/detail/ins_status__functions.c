  ...
  vectornav_msgs__msg__InsStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vectornav_msgs__msg__InsStatus * data =
      (vectornav_msgs__msg__InsStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vectornav_msgs__msg__InsStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          vectornav_msgs__msg__InsStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
