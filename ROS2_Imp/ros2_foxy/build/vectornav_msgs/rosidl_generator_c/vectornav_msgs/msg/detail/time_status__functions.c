  ...
  vectornav_msgs__msg__TimeStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vectornav_msgs__msg__TimeStatus * data =
      (vectornav_msgs__msg__TimeStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vectornav_msgs__msg__TimeStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          vectornav_msgs__msg__TimeStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
